...
Use the contexts of `dbDefault` to build the instance ID.

For instance, on HP machines we want to only offer the 2023 UEFI db update to machines with firmware
new enough to not be bricked. We can identify these by looking at `dbDefault` and matching the
2011 CA in `db` and the 2023 CA in `dbDefault`:
